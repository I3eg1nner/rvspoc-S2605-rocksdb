...
# RISCV_NO_RVV_CRC32C=1 skips the probe entirely: used to build a
# STOCK-EQUIVALENT scalar baseline whose binary contains zero vector
# instructions (the Zvbc TU is compiled with a vector march even in
# PORTABLE builds, relying on the runtime hwprobe/env gate otherwise).
ifneq ($(RISCV_NO_RVV_CRC32C),1)
