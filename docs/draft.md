...
## S0/SP/GP 归因对照（2026-08-29）——headline 重大修正

结果表见 ACCEPTANCE 二节。核心事实：SP（纯标量 O3+PGO 同训练集）
在全部 5 个 headline 点上稳定优于 GP（交付配置），GP/SP =
−3.1%~−9.7%，逐轮符号 6/6、6/6、4/4、3/3、2/3。GP/S0 仅
+2.6%~+15.8%，远低于此前公布的 S vs G 差值。

方法论教训（候选促进 wiki）：**把通用编译优化（O3/PGO）与
架构专有改动捆绑在同一"交付 vs 基线"对比里，会把通用份额误记为
架构收益**——正确做法是三臂（stock 基线 / 基线+通用优化 /
交付），本应在第一次终版组装时就做。外部评审点名要求补此对照，
完全命中。

嫌疑排序（待 V1/V3 归因细分）：全局 rv64gcv march 下 O3 自动向量化
在 X100 上的去优化（向量启动开销 > 短循环收益）> RVV memcmp 在
