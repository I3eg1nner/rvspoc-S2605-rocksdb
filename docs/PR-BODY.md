...
同会话交错配对（协议见 docs/ACCEPTANCE.md 二节）：交付配置 vs
标量 -O2 基线为 fill +10.1% / read t8 +19.3% / seek t8 +16.8% /
read t1 +27.4% / seek t1 +22.7%。

**归因披露（我们主动补测的三臂对照）**：纯标量 O3+PGO（不含任何
riscv 专有改动）即可解释上述增益的大部分；RVV/riscv 专有部分在
K3 这一访存延迟主导的负载上净贡献有限（详表在 ACCEPTANCE）。我们
选择如实呈报并把交付配置按归因数据重组装（终版配方以 REPRODUCE
为准）。LX5000（DDR5、48 核）上的占比无法从 K3 外推。
